"""ETSformer encoder/decoder layers.

Adapted from ``time_series_models_labs/layers/ETSformer_EncDec.py``.

Architecture overview
---------------------
- **GrowthLayer**: exponential smoothing of differenced series → trend
- **FourierLayer**: FFT-based frequency extrapolation → seasonality
- **LevelLayer**: exponential smoothing of level (trend + season removed)
- **Encoder**: stack of EncoderLayer, each producing level, growth, season
- **Decoder**: damping + seasonal horizon extraction
- **Transform**: data augmentation (jitter + scale + shift)
"""
...
class Transform:
    """Random jitter + scaling + shift for training-time augmentation."""
...
class ExponentialSmoothing(nn.Module):
    """Learnable exponential smoothing with FFT convolution."""
...
class GrowthLayer(nn.Module):
    """Exponential smoothing on differenced series → growth/trend."""
...
# ===================================================================
#  Fourier Layer (seasonality)
# ===================================================================

class FourierLayer(nn.Module):
    """FFT-based frequency extrapolation for seasonality."""
...
class LevelLayer(nn.Module):
    """Exponential smoothing of (level - season) with growth as auxiliary."""
...
class EncoderLayer(nn.Module):
    ...
    @property
    def pred_len(self):
        return self.seasonal_layer.pred_len

    def _growth_block(self, x):
        return self.dropout1(self.growth_layer(x))

    def _season_block(self, x):
        return self.dropout2(self.seasonal_layer(x))
...
# ===================================================================
#  Damping Layer (decoder)
# ===================================================================

class DampingLayer(nn.Module):
    """Learnable damping factor applied to growth over the prediction horizon."""
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, growth, season):
        growth_horizon = self.dropout1(self.growth_damping(growth[:, -1:]))
        seasonal_horizon = season[:, -self.pred_len:]
        return growth_horizon, seasonal_horizon

    @property
    def pred_len(self):
        return self.growth_damping.pred_len
