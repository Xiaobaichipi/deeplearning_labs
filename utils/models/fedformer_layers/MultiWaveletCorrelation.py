"""Multi-wavelet attention blocks for FEDformer.

Adapted from ``time_series_models_labs/layers/MultiWaveletCorrelation.py``.
Requires ``sympy`` for Legendre/Chebyshev polynomial computation.
"""
...
class FourierCrossAttentionW(nn.Module):
    """Fourier cross attention variant used internally by MultiWaveletCross."""

    def __init__(self, in_channels, out_channels, seq_len_q, seq_len_kv, modes=16,
                 activation='tanh', mode_select_method='random'):
        super().__init__()
...
class MultiWaveletTransform(nn.Module):
    """1D multiwavelet block (self-attention)."""
...
class MultiWaveletCross(nn.Module):
    """1D multiwavelet cross-attention block."""

    def __init__(self, in_channels, out_channels, seq_len_q, seq_len_kv, modes, c=64,
                 k=8, ich=512, L=0, base='legendre', mode_select_method='random',
                 initializer=None, activation='tanh', **kwargs):
        super().__init__()
        self.c = c
        self.k = k
        self.L = L
        ...
        self.attn1 = FourierCrossAttentionW(
            in_channels=in_channels, out_channels=out_channels, seq_len_q=seq_len_q,
